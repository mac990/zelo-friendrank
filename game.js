...
(() => {
  ...
  const ROOT_SELECTOR = "#zelo-liff-game";
  const DAILY_PLAY_LIMIT = 9999;
  const DAILY_LIMIT = DAILY_PLAY_LIMIT;
  ...
  const STORAGE = {
    ...
    selectedTop: "zelo_selected_top_type",
    ...
    inviterCode: "zg_inviter_referral_code",
    myReferralCode: "zg_referral_code",

    lineInviteFriendCount: "zg_line_invite_friend_count",
    fallbackReferralCount: "zg_referral_success_count"
  };
  ...
  function renderHome() {
    ...
    bindOnce("#zg-start-btn", "click", async () => {
      ...
      const canPlay = await ensureCanPlayBeforeStart();
  ...
  function getRoot() {
    return document.querySelector(ROOT_SELECTOR) || document.getElementById("zelo-liff-game");
  }
  ...
  function setRootHtml(html) {
    ...
    if (!root) {
      console.error("[ZELO GAME] root not found:", ROOT_SELECTOR);
      return;
    }
