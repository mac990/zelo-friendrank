...
function ensureResultDom(root) {
  ...
  const score =
    Number(result.score ?? result.points ?? 0) || 0;
  ...
  if (!hasMe) {
    ...
    rows = rows.map((item) => {
      ...
      return {
        ...
        bestScore: Number(item.bestScore || item.score || score)
      };
    });
  }
